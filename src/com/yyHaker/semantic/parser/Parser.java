package com.yyHaker.semantic.parser;


import  com.yyHaker.semantic.lexer.*;
import  com.yyHaker.semantic.inter.*;
...
public class Parser {
   ...
   private Token look;   // lookahead token    向前看的词法单元
   Env top = null;       // current or top symbol table    当前或者顶层的符号表

  public static  int used = 0;         // storage used for declarations  用于变量声明的存储位置
   ...
   void move() throws IOException {
    ...
    void error(String s) {
       ...
       Node.semanticErrorList.add(new SyntaxErrorProperty(lex.line,"",s));
    }

   void match(int t) throws IOException {
      if( look.tag == t ) move();
      else error("无法匹配的语法错误");
   }
   ...
   Stmt assign() throws IOException {
      ...
      if( id == null ){
         Word word=(Word)t;
         error("变量"+word.toString()+ "未声明");
          id=new Id(word,Type.Int,used);
      }
